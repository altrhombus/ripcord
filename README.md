...
- **1,017 unit tests** across two suites, pure managed and cross-platform — they need no console, no GPU and
  no Windows-only hardware. A clean checkout without the authors' captures runs 976 of them and skips 41, by
  ...
  Forty-nine of them — five checks and a forty-four-row detector contract — sweep the published tree and
  every commit message in the history for material this project's redaction policy forbids.
