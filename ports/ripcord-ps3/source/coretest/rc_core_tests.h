...
#define RC_CORE_TEST_COUNT 11

/*
 * WHERE THE .kat FILES LIVE ON THE CONSOLE. Four of the eleven runners check the core against
 * known-answer vectors emitted by the .NET side rather than against logic they carry themselves, so
 * those files have to be copied across. They are dirty-room material - gitignored, generated by
 * `ProtocolLab vectors` - and are not embedded in the binary for the same reason the H.264 capture is
 * not.
 *
 * A missing file is not a failure. The runner reports it and the suite is counted as skipped, which is
 * the same contract ecdh_test already has on the host when its backend is absent.
 */
#ifndef RC_CORE_TEST_VECTOR_DIR
#define RC_CORE_TEST_VECTOR_DIR "/dev_hdd0/ripcord-vectors/"
#endif

typedef struct {
    const char *name;
    int         exit_code;   /* 0 = every assertion in that runner passed */
    int         skipped;     /* the vectors it needs were not on the console */
} rc_core_test_result;
